...
function prepareContent(results, templates, source) {
    var bodyContent = '',
        items = '',
        previousFile = '',
        content = '';
    ...
    results.forEach(function(element) {
        ...
        errReason = error.reason || (error.rule && error.rule.description) || (error.rule && error.rule.desc) || "";
...
function prepareSummary(results, templates, source) {
...
module.exports = function getRenderedHTML(results, source) {
    var templates = loadTemplates();

    return templates.body
        .replace('{content}', prepareContent(results, templates, source))
        .replace('{summary}', prepareSummary(results, templates, source));
}
